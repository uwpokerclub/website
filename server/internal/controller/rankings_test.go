...
func TestListRankingsSearch(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			reqURL := fmt.Sprintf("/api/v2/semesters/%s/rankings?%s", testutils.TEST_SEMESTERS[0].ID, params.Encode())

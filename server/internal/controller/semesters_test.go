...
func TestCreateSemester(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:     "successful request with zero fees",
			userRole: authorization.ROLE_PRESIDENT.ToString(),
			requestBody: map[string]interface{}{
				"name":                  "Winter 2025",
				"startDate":             "2024-12-31T19:00:00-05:00",
			...
			expectedResponse: map[string]interface{}{
				...
				"startingBudget":        float64(0),
				"currentBudget":         float64(0),
				"membershipFee":         float64(0),
				"membershipDiscountFee": float64(0),
				"rebuyFee":              float64(0),
				"freeTrialLimit":        float64(0),
				"id":                    nil,
			},
		...
		{
			name:     "invalid request missing membership fee",
			userRole: authorization.ROLE_PRESIDENT.ToString(),
			requestBody: map[string]interface{}{
				"name":                  "Fall 2024",
				"startDate":             "2024-09-01T00:00:00Z",
			...
			expectedStatus:   http.StatusBadRequest,
			expectError:      true,
			expectedErrorMsg: "Key: 'CreateSemesterRequest.MembershipFee' Error:Field validation for 'MembershipFee' failed on the 'required' tag",
		},
		{

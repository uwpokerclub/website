...
export default function useSelect({
  ...
  const middleware = [useFuzzySearch ? fuzzySearch : null, ...(filterOptions ? filterOptions : [])];
  const filteredOptions = groupOptions(reduce(middleware, options, q));
  const [keyHandlers, highlighted, setHighlighted] = useHighlight(filteredOptions, onSelect, ref);
  ...
  useEffect(() => {
    setOption(getOption(value === undefined ? defaultValue : value, options));
